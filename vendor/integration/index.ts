  ...
  return {
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        const buildLogger = logger.fork('rvbcorp');

        const virtualModuleId = 'rvbcorp:config';
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              {
                name: 'vite-plugin-rvbcorp-config',
                resolveId(id) {
        ...
        if (typeof _themeConfig === 'string') {
          ...
          buildLogger.info(`RVB Corp \`${_themeConfig}\` has been loaded.`);
        } else {
          buildLogger.info(`RVB Corp config has been loaded.`);
        }
      },
